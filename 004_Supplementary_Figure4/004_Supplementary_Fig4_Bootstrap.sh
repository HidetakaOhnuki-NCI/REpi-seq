...
# Step8: Add a header to the BED files for the subsequent bootstrap statistical test.
...
# Step13: Run swarm command for executing the R script in each directory.
# The bootstrap statistical test was performed for 50,000 bins. 
# The upper limit of memory and threads in NIH high-performance computers are 240 GB memory/job and 50 threads/job. 
# To avoid crushing the job, the approximate upper limit of bins is 50,000 bins/job.
...
# Step17: Calculate the difference between antibody and control IgG in each putative signal region.
...
# Step18: Calculate the total number of putative signals in each single cell.
...
# Summarize the number of signal regions in each cell.
